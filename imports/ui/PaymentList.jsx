                            ...
                            {this.props.products.filter(product => product.packagings().filter(packaging => packaging.retrieved() > 0).length > 0).map(product => (
                                <React.Fragment key={product._id}>
                                    ...
                                    {product.packagings().filter(packaging => packaging.retrieved() > 0).map(packaging => (
                                        <PaymentListItem key={packaging._id} packaging={packaging} value={this.state.price[packaging._id]} onChange={event => this.setState({price: { ...this.state.price, [packaging._id]: event.target.value}})}/>
                                    ))}
                                </React.Fragment>
                            ))}
                        </List>
